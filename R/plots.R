...
fertile_ratio <- function(e0_f, e0_m, SRB = 1.05, ALB = 40, max_age = 60){
  ...
  n_men_fertile / n_women_fertile
  
}

# Compute ratio of fertile females to adult males
fertile_params <- expand_grid(e0_f = 30:70, e0_m = 25:60)
fertile_params$ratio <- map2_dbl(fertile_params$e0_f, fertile_params$e0_m, fertile_ratio, .progress = T)

plot_fertile_ratio <- 
  fertile_params |> 
  dplyr::filter(e0_f <= 45, e0_m <= 40) |> 
  ggplot(aes(e0_f, e0_m, fill = ratio)) + 
  geom_raster() + 
  geom_textcontour(aes(z = ratio, label = after_stat(scales::number(level, accuracy = 0.01))), colour = 'black', alpha = 0.75, hjust = 0.55) +
  ...
  geom_textabline(intercept = -10, slope = 1, colour = 'yellow', label = '10 year difference', hjust = 0.8) +
  scale_fill_scico(palette = 'vik', midpoint = 1, direction = 1, limits = c(0.90, 2.3)) +
  guides(fill = guide_colorbar(title = 'Ratio', reverse = F)) +
  labs(
    title = "Ratio of fertile men (18-60) to fertile women (18-40)",
  ...
  annotate("text", x = 35.5, y = 30, label = "Current study", hjust = 0, colour = "white") +
  coord_fixed() +
